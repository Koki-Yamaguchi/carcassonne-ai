...
pub fn decode(file_path: String) -> Vec<Move> {
  ...
  let mut moves = vec![
    Move::TMove( TileMove { ord, game_id, player_id: player_id, tile: Tile::StartingTile, rot: 0, pos: (0, 0) } ),
    Move::MMove( MeepleMove { ord: ord + 1, game_id, player_id: player_id, meeple_id: -1, tile_pos: (0, 0), meeple_pos: -1 } ),
  ];
  ord += 2;
          ...
          Some(t) => {
            match t {
              "playTile" => {
                ...
                match prev_move {
                  Move::TMove(t) => {
                    moves.push(Move::MMove( MeepleMove { ord, game_id, player_id, meeple_id: -1, tile_pos: t.pos, meeple_pos: -1 } ));
                    ord += 1;
                  }
                  _ => {}
                }
                ...
                let tile_type = packet["data"][0]["args"]["type"].as_str().unwrap().parse().unwrap();
                let y = packet["data"][0]["args"]["y"].as_str().unwrap().parse().unwrap();
                let x = packet["data"][0]["args"]["x"].as_str().unwrap().parse().unwrap();
                let rot = packet["data"][0]["args"]["ori"].as_str().unwrap().parse::<i32>().unwrap() - 1;
                tile_id = convert_tile(tile_type);

                moves.push(Move::TMove( TileMove { ord, game_id, player_id, tile: to_tile(tile_id), rot, pos: (y, x) } ));
              ...
              "playPartisan" => {
                let y = packet["data"][0]["args"]["y"].as_str().unwrap().parse().unwrap();
                let x = packet["data"][0]["args"]["x"].as_str().unwrap().parse().unwrap();
                let pos = packet["data"][0]["args"]["pos"].as_str().unwrap().parse().unwrap();

                if remaining_meeples[player_id as usize].len() == 0 {
                  panic!("decode failed: no meeple is available");
                }
                let meeple_id = remaining_meeples[player_id as usize].iter().next().unwrap().clone();
                remaining_meeples[player_id as usize].remove(&meeple_id);

                moves.push(Move::MMove( MeepleMove { ord, game_id, player_id, meeple_id, tile_pos: (y, x), meeple_pos: convert_pos(tile_id, pos) } ));
